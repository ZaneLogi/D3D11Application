...
bool LightShader::initialize(ID3D11Device* device, HWND hwnd)
{
    ...
    result = initialize_shader(device, hwnd, L"light.vs", L"light.ps");
    if (!result)
    {
        return false;
    }
    ...
    XMFLOAT4 diffuseColor)
{
    ...
    result = set_shader_parameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture, lightDirection, diffuseColor);
    if (!result)
    {
        return false;
    }
...
bool LightShader::initialize_shader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
    HRESULT result;
...
void LightShader::shutdown_shader()
{
...
void LightShader::output_shader_error_message(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
    char* compileErrors;
...
bool LightShader::set_shader_parameters(
    ID3D11DeviceContext* deviceContext,
    const XMMATRIX& worldMatrix,
    const XMMATRIX& viewMatrix,
    const XMMATRIX& projectionMatrix,
...
void LightShader::render_shader(ID3D11DeviceContext* deviceContext, int indexCount)
{
