...
bool RectangleModel::initialize(
    ID3D11Device* device,
...
void RectangleModel::set_screen_size(int width, int height)
{
    m_screenWidth = width;
...
bool RectangleModel::render(ID3D11DeviceContext* deviceContext, int positionX, int positionY)
{
    // Re-build the dynamic vertex buffer for rendering to possibly a different location on the screen.
    if (!update_buffers(deviceContext, positionX, positionY))
    {
        return false;
    }
...
int RectangleModel::get_index_count()
{
    return m_indexCount;
}

void RectangleModel::get_world_matrix(XMMATRIX& mat)
{
    mat = XMMatrixIdentity();
}

bool RectangleModel::initialize_buffers(ID3D11Device* device)
{
    VertexType* vertices;
...
void RectangleModel::shutdown_buffers()
{
...
bool RectangleModel::update_buffers(ID3D11DeviceContext* deviceContext, int positionX, int positionY)
{
    ...
    right = left + (float)m_rectWidth;

    // Calculate the screen coordinates of the top of the bitmap.
    top = (float)(m_screenHeight / 2) - (float)positionY;

    // Calculate the screen coordinates of the bottom of the bitmap.
    bottom = top - (float)m_rectHeight;
...
void RectangleModel::render_buffers(ID3D11DeviceContext* deviceContext)
{
    unsigned int stride;
    unsigned int offset;
